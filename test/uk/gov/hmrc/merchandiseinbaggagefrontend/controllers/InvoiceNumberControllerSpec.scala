  ...
  "onSubmit" must {
    "return BAD_REQUEST and errors" when {
      "no selection is made" in {
        ...
        val result = controller.onSubmit(1)(buildPost(routes.InvoiceNumberController.onSubmit(1).url, sessionId))
        val content = contentAsString(result)

        status(result) mustEqual BAD_REQUEST

        content must include(s"What is the invoice number for the ${startedGoodsEntry.maybeCategoryQuantityOfGoods.get.category}?")
