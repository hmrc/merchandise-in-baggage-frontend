  ...
  "onSubmit" must {
    "return BAD_REQUEST and errors" when {
      "no selection is made" in {
        ...
        val result = controller.onSubmit(1)(buildPost(routes.RemoveGoodsController.onSubmit(1).url, sessionId))
        val content = contentAsString(result)

        status(result) mustEqual BAD_REQUEST

        content must include(s"Are you sure you want to remove the ${completedGoodsEntry.maybeCategoryQuantityOfGoods.get.category}?")
